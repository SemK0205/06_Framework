...
if (updateInfo != null) {
    ...
    updateInfo.addEventListener("submit", async e => {
        ...
        // 기존 닉네임이 저장되어 있는 요소의 value값 얻어오기
        ...
        if (currentNickname !== memberNickname.value) {
            ...
            const resp = await fetch("/member/checkNickname?=" + memberNickname.value);
        ...
        e.preventDefault()가 비동기 함수 이후에 호출되면 이미 form 이 submit되어버린 다음일 수 있음
    ...
    imageInput.addEventListener("change", () => {
        ...
        if(file) { // 파일이 선택된 경우
            ...
            if(file.size <= MAX_SIZE) { // 현재 선택한 파일의 크기가 허용범위 이내인 경우(정상인 경우)
                ...
                // 웹에서 접근 가능한 임시 URL 반환
                console.log(newImageUrl); // blob:http://localhost/2a55df7e-d0fd-4887-8799-270e03b55fc2
                profileImg.src = newImageUrl; // 미리보기 이미지 설정 (img 태그의 src에 선택한 파일 임시 경로 대입)
                previousImage = newImageUrl; // 현재 선택된 이미지를 이전 이미지로 저장
