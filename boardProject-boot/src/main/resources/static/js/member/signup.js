...
sendAuthKeyBtn.addEventListener("click", () => {
  ...
  // 새로 발급받은 인증번호 확인 전 까진 checkObj.authKey 는 false
  checkObj.authKey = false;
  // 인증번호 발급 관련된 메시지 지우기
  authKeyMessage.innerText = "";
  ...
  // - 지연시간(ms)만큼 시간이 지날 때 마다 콜백함수 수행
...
checkAuthKeyBtn.addEventListener("click", () => {
  ...
  if (authKey.value.length < 6 || authKey.value.length > 6) { // 인증번호가 제대로 입력 안된 경우 (길이가 6미만 또는 7이상)
  ...
  fetch("/email/checkAuthKey", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(obj) // obj 라는 JS 객체를 JSON으로 변경
  }).then(resp => resp.text())
    .then(result => {
      // 1 or 0
...
memberNickname.addEventListener("input", (e) => {
  ...
  fetch("/member/checkNickname?memberNickname=" + inputNickname)
    ...
    .then(result => {
      if (result > 0) { // 중복
        nickMessage.innerHTML = "중복된 닉네임 입니다.";
        nickMessage.classList.add("error");
        nickMessage.classList.remove("confirm");
        checkObj.memberNickname = false;
        return;
      ...
      nickMessage.innerHTML = "사용 가능한 닉네임 입니다.";
...
memberTel.addEventListener("input", (e) => {
  ...
  // 2) 정규식 검사
  const regExp = /^01[016789][0-9]{7,8}$/;

  if (!regExp.test(inputTel)) {
    telMessage.innerHTML = "올바른 형식의 전화번호를 입력해주세요.";
  ...
  // 3) 전화번호 중복 검사
  fetch("/member/checkMemberTel", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ memberTel: inputTel })
  })
    .then(resp => resp.text())
    .then(result => {

      if (result > 0) {
        telMessage.innerHTML = "중복된 전화번호 입니다. 다른 번호를 입력해주세요.";
        telMessage.classList.add("error");
        telMessage.classList.remove("confirm");
        checkObj.memberTel = false;
        return;
      ...
      telMessage.innerHTML = "사용 가능한 전화번호 입니다.";
...
signUpform.addEventListener("submit", (e) => {
  ...
  for(let key in checkObj) { // checkObj 요소의 key값을 순서대로 꺼내옴
    ...
    if(!checkObj[key]) { // 현재 접근중인 checkObj[key] 의 value 값이 false인 경우
