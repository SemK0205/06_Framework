...
// 구분 하여 (REpresentational) 자원의 상태(State)를 주고 받는 것(Transfer)

// -> 자원의 이름(주소)를 명시하고
//      HTTP method(GET,POST,PUT,DELETE)를 이용해 지정된 자원에 대한 CRUD 진행
...
addComment.addEventListener("click", () => {
    ...
    if (commentContent.value.trim().length == 0) {
        alert("내용 작성 후 등록버튼 클릭해 주세요");
        commentContent.focus();
        return;
    }

    // 로그인이 되어있는 경우 + 댓글 내용이 작성되어 있는 경우
    // ajax를 이용해 댓글 등록 요청
    const data = {
        "boardNo": boardNo,
