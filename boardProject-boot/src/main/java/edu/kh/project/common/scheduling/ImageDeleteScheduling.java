...
public class ImageDeleteScheduling {
	...
	public void scheduling() {
		...
		// 두 배열을 하나로 합침
		// imageArr 라는 빈 배열을 boardArr과 memberArr 의 길이 만큼의 크기로 만든다.
		File[] imageArr = new File[boardArr.length + memberArr.length];
		
		// 배열 내용 복사(깊은 복사)
		// System.arraycopy(복사할 배열, 몇번인덱스부터 복사 할지,
		// 					새로운 배열, 새로운배열의 몇 번 인덱스부터 넣을지,
		//					복사를 어디까지 할 건지)
...
*
*
*  @Scheduled(cron = "30 * * * * *") // 시계 초 단위가 30인 경우 수행
*  @Scheduled(cron = "0,30 * * * * *") // 시계 초 단위가 30인 경우 수행
*  @Scheduled(cron = "0 0 12 * * *") // 정오
*  @Scheduled(cron = "0 0 0 * * *") // 자정
*  @Scheduled(cron = "0 0 * * * *") // 매 시간(정시)마다 수행
*  @Scheduled(cron = "0 0 0 1 * *") // 매 월 마다
*
*
* - 특수문자
* * : 모든 수.
