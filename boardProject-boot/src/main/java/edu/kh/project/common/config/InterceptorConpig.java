...
import edu.kh.project.common.interceptor.BoardTypeIntercepter;
...
public class InterceptorConpig implements WebMvcConfigurer {
	...
	public void addInterceptors(InterceptorRegistry registry) {
		...
		registry
		.addInterceptor(boardTypeIntercepter())
		.addPathPatterns("/**") // 가로챌 요청 주소를 지정 /** : / 이하 모든 요청 주소
		.excludePathPatterns("/css/**", "/js/**", "/images/**", "/favicon.ico"); // 가로채지 않을 주소를 지정 (정적요청은 제외 시키기)
