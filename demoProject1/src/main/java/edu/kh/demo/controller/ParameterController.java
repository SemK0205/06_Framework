...
public class ParameterController {
  ...
  public String paramTest1(HttpServletRequest req) {
    ...
    log.debug("inputAdress : " + inputAddress);
   ...
   * 
   * [속성 추가 작성법]
   * @RequestParam(value="key", required=true, defaultValue="1") 자료형 매개변수명
   * 
   * value : 전달받은 input 태그의 name 속성값(파라미터 key)
   * required : 입력된 name 속성값 파라미터 필수 여부 지정(기본값 true)
   * -> required=true인 파라미터가 존재하지 않는다면 400(Bad Request) 에러 발생
   * -> ""(빈 문자열)일 때는 에러 발생 X (파라미터가 존재하지 않는것이 아니라 name 속성값이 ""인 것)
   * 
   * defaultValue : 파라미터 중 일치하는 name속성값이 없을 경우에 대입할 값 지정
   * -> required가 false인 경우 사용
   */
  @PostMapping("test2") // /param/test2 POST 방식 요청 매핑하는 매서드다!
  public String paramTest2(@RequestParam("title") String title, 
                          @RequestParam("writer") String writer,
                          @RequestParam("price") int price, // 형변환이 필요한 경우에는 파라미터 value값 필수 작성되어있어야함!
  ...
  // 3. @RequestParam 어노테이션 - 여러 개의 파라미터
  ...
  public String paramTest3( @RequestParam("color") String[] colorArr,
    ...
    // -> 같은 name 속성값을 가진 파라미터는 String[], List로 저장 X
  ...
  public String paramTest4(/*@ModelAttribute*/ MemberDTO inputMember) {
    ...
    // == "redirect:/param/main"
    // (현재위치가 중요! 
    // -> 현재 경로의 가장 마지막 부분을 redirect:이주소 로 변경하기 때문에)
  }
